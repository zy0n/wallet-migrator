...
function parseWeiToFloat(bigNumber) {
  return parseFloat(
    (ethers.utils.parseUnits(bigNumber.toString(), "wei") / 10 ** 18).toString()
  );
}
...
function outputLogNFTs(n) {
  return n.erc1155
    ? {
        ...
        ids_balances: {
          ids: n.ids,
          balances: n.balances.map((b) => parseFloat(b.toString())),
        },
        address: n.address,
      }
    : {
...
async function outputAssetLogs(
  _tokens,
  _token_errors,
  _nfts,
  _nft_errors,
  ...
  const out_str = JSON.stringify(
    {
      tokens: out_tokens,
      nfts: out_nfts,
      errors: { tokens: out_token_errors, nfts: out_nft_errors },
    },
    null,
    4
  );
  const out_path = path.join(
    process.cwd(),
    "debug",
    `${_ogWallet.provider}_${originWallet.address}_assets.json`
  );
...
async function outputRandomWallet(walletName, originAddress) {
  ...
  const walletPath = path.join(
    ...
    ".secret",
    `${
      USE_MULTI ? `${_walletName}_${originAddress}` : `${_walletName}`
    }_info.json`
  );
...
async function getERC20Balance(tokenAddress, balanceAddress, provider) {
  if (
    ...
    !ethers.utils.isAddress(balanceAddress)
  ) {
...
async function getTokenBalances(walletAddress, provider, providerInfo) {
  ...
  for (let i = 0; i < transactions.length; i++) {
    ...
    printLn(
      `Scanning Tx ${i + 1} of ${transactions.length} | 👍 ${tokens.length} `
    );
    const txValue = ethers.utils.parseUnits(
      tx.value,
      parseInt(tx.tokenDecimal)
    );
    const scan_condition = SCAN_ALL
      ? true
      : txValue.gt(0) && typeof sent_to[tx.to] != "undefined" && sent_to[tx.to];
    if (
      !used_tokens[tokenAddress] &&
      (scan_condition || TOKENS_TO_KEEP.indexOf(tx.tokenSymbol) != -1)
    ) {
      const new_balance = await getERC20Balance(
        tokenAddress,
        walletAddress,
        provider
      );
      const newBalance = ethers.utils
        .parseUnits(new_balance.toString(), tx.tokenDecimal)
        .toString();
      const readableBalance = parseFloat(
        (
          ethers.utils.parseUnits(new_balance.toString(), `wei`) /
    ...
    try {
      if (token.erc1155) {
        if (token.ids.length > 0) {
          ...
          const tx = await contract.estimateGas.safeBatchTransferFrom(
        ...
        const tx = await contract.estimateGas.safeTransferFrom(
          originWallet.address,
          toAddress,
          token.tokenId
        );
...
async function checkTokenGas(tokens, toAddress, originWallet) {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    try {
      ...
      const tx = await contract.estimateGas.transfer(
        toAddress,
        token.bigBalance
      );
      totalGas = totalGas.add(tx.mul(gasPrice));
...
async function moveTokensToAddress(tokens, toAddress, originWallet) {
  for (let i = 0; i < tokens.length; i++) {
    try {
      ...
      console.log(
        `${DRY_RUN ? "👀 Faux Sent" : "💰 Sent"} ${token.readableBalance} ${
          token.symbol
        }`
      );
    } catch (error) {
    ...
    try {
      ...
      if (token.erc1155) {
        if (token.ids.length > 0) {
          ...
          console.log(
            `${DRY_RUN ? "👀 Faux Sent" : "💰 Sent"} ${token.symbol} #${
              token.ids
            } qty:(${token.balances})`
          );
        }
...
async function sendChainBalance(
  provider,
  walletInfo,
  chain_balance,
  originWallet
  ...
  try {
    ...
    console.log(
      "⛔️ There was an error processing that transaction, please try again."
    );
...
async function runTheMagic() {
  for (let i = 0; i < ORIGIN_WALLETS.length; i++) {
    ...
    const walletInfo = await outputRandomWallet(
      _ogWallet.prefix || "secret",
      originWallet.address
    );
    ...
    console.log(
      _ogWallet.provider.toUpperCase(),
      "|",
      originWallet.address,
      `(${parseFloat(current_balance)} ${providerInfo.symbol})`
    );
    ...
    if (nfts.length > 0) {
      ...
      const { gasCost, readable, out_nfts, nft_errors } = await checkNFTGas(
        nfts,
        walletInfo.address,
        originWallet
      );
      ...
      if (now_balance.gt(gasCost)) {
        if (_nfts.length > 0) {
          ...
          console.log(
            "🥀 No NFTs to send? Check for errored NFTs in the debug log. 👀"
          );
        }
      } else {
        ...
        console.log(
          `⛔️ NFTS: Not enough gas. Needs: ${gas_needed.toString()} ${
            providerInfo.symbol
          }`
        );
      }
    ...
    if (tokens.length > 0) {
      ...
      const { gasCost, readable, out_tokens, token_errors } =
        await checkTokenGas(tokens, walletInfo.address, originWallet);
      _tokens = out_tokens;
      ...
      if (now_balance.gt(gasCost)) {
        if (_tokens.length > 0) {
          ...
          console.log(
            "🥀 No Tokens to send? Check for errored Tokens in the debug log. 👀"
          );
        }
      } else {
        const needed = gasCost.sub(now_balance);
        const gas_needed = parseWeiToFloat(needed);
        console.log(
          `⛔️ TOKENS: Not enough gas. Needs: ${gas_needed.toString()} ${
            providerInfo.symbol
          }`
        );
      }
    }
    await outputAssetLogs(
      _tokens,
      _token_errors,
      _nfts,
      _nft_errors,
    ...
    if (CLEAR_WALLET) {
      ...
      if (remains > 0) {
        console.log(
          `👀 There is still (${remains} ${providerInfo.symbol}) left on this account.`
        );
      } else {
        console.log(
          `✅ There's no more ${providerInfo.symbol} on this account.`
        );
      }
    }
  }
      ...
      if (a.startsWith("--")) {
        switch (a) {
          ...
          case "--wallet-prefix":
            ...
            if (
              (!prefix_override.startsWith("--") && prefix_override != "") ||
              prefix_override != " "
            ) {
